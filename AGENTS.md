...
| 원칙 | 구현 |
|------|------|
| Agent 조직 구조 | 6개 전문 agent + 오케스트레이터 |
...
**목표:** 트렌드 리서치부터 발행·성과 분석까지 블로그 운영 전 과정을 5개 전문 에이전트가 자동화한다.
...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-05-02 | 초기 하네스 구성 | 전체 | agent_prompts/ 기반 .claude/agents/ 및 skills/ 신규 생성 |
| 2026-05-02 | 디자인 에이전트 레퍼런스 연결 | CLAUDE.md, guide/design_agent/reference.md | 블로그 디자인 요청 시 참고 사이트와 추출 기준 고정 |
| 2026-05-02 | design_agent 오케스트레이터 팀 등록 | .claude/skills/blog-orchestrator/SKILL.md | 에이전트 팀 섹션 누락 수정 |
| 2026-05-02 | image_agent 신규 추가 | .claude/agents/, agent_prompts/, scripts/core/, guide/image_agent/, inputs/blog_config.yaml, workflows.md, routing.md | 이미지 수동 준비 단계 자동화 (DALL-E 3 / Stable Diffusion 추상화 레이어, 선택적 파이프라인 단계) |
| 2026-05-02 | design_agent HTML 패치 규칙 추가 | agent_prompts/design_agent.md, CLAUDE.md | 검색/최근글 컴포넌트 소실 사고 재발 방지: HTML 전체 교체 원칙, 사이드바 6종 diff 체크리스트, s_sidebar_element 배치 규칙, CSS 타임아웃 수동 적용 안내 |

---

상세 참조: 스키마 `guide/operations/schema.md` | 에러 처리 `guide/operations/error_guide.md` | 디자인 `guide/design_agent/reference.md`
